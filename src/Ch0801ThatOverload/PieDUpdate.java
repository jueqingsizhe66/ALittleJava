...
class Bot extends PieDUpdate {
    ...
    PieDUpdate accept(PieVistor1 ask) {
        ...
        return ask.forPieD(this); // Interesting part, 通过this调用field
    }
...
//    PieDUpdate forTop(Top that);
     PieDUpdate forPieD(PieDUpdate that);
}
...
class RemV implements PieVistor1 {
    ...
    //为什么要改为Public???
    public PieDUpdate forPieD(Bot that){
        return new Bot();
    }
    //改用Object即可
    //public PieDUpdate forTop(Object t, PieDUpdate r){ //不能用int
     public PieDUpdate forPieD(Top that){ //不能用int
        if(o.equals(that.t)){
...
class SubstV implements PieVistor1 {
    ...
    public PieDUpdate forPieD(Top that){
        if(o.equals(that.t)){
